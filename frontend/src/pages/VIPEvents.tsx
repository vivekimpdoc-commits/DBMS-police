  ...
  return (
    ...
    >
      <motion.div
        ...
        className="glass-card rounded-3xl p-8 w-full max-w-lg shadow-[0_25px_50px_-12px_rgba(0,0,0,0.8)]"
      >
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-xl bg-slate-800/50 hover:bg-slate-700 flex items-center justify-center text-slate-400 hover:text-white transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-2xl text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-all"
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-2xl text-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-all"
              />
            </div>
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-2xl text-white focus:outline-none focus:border-blue-500 transition-all appearance-none"
              >
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-2xl text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-all"
            />
          </div>
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 bg-slate-800/50 border border-slate-700 rounded-2xl text-white focus:outline-none focus:border-blue-500 transition-all appearance-none"
            >
              <option value="Planning" className="bg-slate-800 text-white">Planning</option>
              <option value="ASL Pending" className="bg-slate-800 text-white">ASL Pending</option>
...
export default function VIPEvents() {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <motion.div key={i} whileHover={{ y: -3 }}
            className={`p-4 rounded-2xl border bg-gradient-to-br ${s.bg} ${s.border} glass-card hover:border-white/20`}
          >
      ...
      {/* Table Card */}
      <div className="glass-card rounded-3xl overflow-hidden shadow-2xl flex-1 flex flex-col">
        {/* Toolbar */}
        <div className="p-4 border-b border-white/5 flex flex-wrap gap-3 items-center">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-800/50 border border-slate-700 rounded-2xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/20 transition-all"
            />
          </div>
          <div className="flex gap-2">
            {['All', 'High', 'Medium', 'Low'].map(t => (
              <button key={t}
                ...
                className={`px-3 py-2 rounded-xl text-xs font-bold transition-all border ${filterThreat === t ? 'bg-blue-600 text-white border-blue-500' : 'bg-slate-800/50 text-slate-400 border-slate-700 hover:border-slate-500'}`}
              >
        ...
        <div className="overflow-x-auto flex-1">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-900/40 border-b border-white/5 text-slate-400 text-[11px] uppercase tracking-widest font-bold">
        ...
        <div className="p-4 border-t border-white/5 flex justify-between items-center">
          <p className="text-xs text-slate-400 font-medium">
            Showing <span className="text-white font-bold">{filtered.length}</span> of <span className="text-white font-bold">{events.length}</span> events
          </p>
          <div className="flex gap-1.5">
            {['Prev', '1', 'Next'].map((b) => (
              <button key={b} className={`px-3 py-1.5 text-xs rounded-xl font-bold border transition-colors ${b === '1' ? 'bg-blue-600 text-white border-blue-500' : 'bg-slate-800/50 text-slate-400 border-slate-700 hover:bg-slate-700'}`}>{b}</button>
            ))}
          </div>
        </div>
      </div>
