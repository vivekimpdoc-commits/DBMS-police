...
const rankColors: Record<string, string> = {
  DGP: 'bg-purple-50 text-purple-700 border-purple-200',
  ADG: 'bg-purple-50 text-purple-700 border-purple-200',
  IG:  'bg-indigo-50 text-indigo-700 border-indigo-200',
  DSP: 'bg-blue-50 text-blue-700 border-blue-200',
  Inspector: 'bg-teal-50 text-teal-700 border-teal-200',
  'Sub-Inspector': 'bg-cyan-50 text-cyan-700 border-cyan-200',
  'Head Constable': 'bg-slate-50 text-slate-700 border-slate-200',
  Constable: 'bg-slate-50 text-slate-600 border-slate-200',
};

const availColors: Record<string, string> = {
  Available: 'text-emerald-700 bg-emerald-50 border-emerald-200',
  Deployed:  'text-orange-700 bg-orange-50 border-orange-200',
  'On Leave': 'text-red-700 bg-red-50 border-red-200',
};
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.9, y: 24 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 24 }}
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="text-xl font-black text-slate-900">नया अधिकारी जोड़ें</h3>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-4">
          {[
            ...
            <div key={f.key} className={f.col === 2 ? 'col-span-2' : ''}>
              ...
              <input type="text" required placeholder={f.placeholder}
                ...
                className="w-full px-4 py-3 bg-white border border-slate-200 rounded-2xl text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 transition-all text-sm shadow-sm"
              />
            </div>
          ))}
          ...
          <div className="col-span-2 flex gap-3 pt-2">
            ...
            <motion.button whileTap={{ scale: 0.97 }} type="submit" disabled={saving}
              className="flex-1 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white rounded-2xl font-bold text-sm transition-all shadow-lg shadow-emerald-500/30 disabled:opacity-60">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-3xl font-black text-slate-800">Officers Registry</h2>
      ...
      {/* Filters + Table */}
      <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden flex-1 flex flex-col shadow-sm">
          ...
          <div className="flex gap-2 flex-wrap">
            {['All', 'Available', 'Deployed', 'On Leave'].map(a => (
              <button key={a} onClick={() => setFilterAvail(a)}
                className={`px-3 py-2 rounded-xl text-xs font-bold border transition-all ${filterAvail === a ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300 shadow-sm'}`}>
                {a}
              </button>
            ))}
            <select value={filterRank} onChange={e => setFilterRank(e.target.value)}
              className="px-3 py-2 bg-white text-slate-500 border border-slate-200 rounded-xl text-xs font-bold focus:outline-none hover:border-slate-300 transition-all appearance-none shadow-sm">
              ...
              {loading ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i} className="border-b border-slate-100">
                    {[...Array(8)].map((_, j) => (
                      <td key={j} className="p-4"><div className="h-4 bg-slate-100 rounded-full animate-pulse" /></td>
                    ))}
                  </tr>
                ))
              ) : filtered.length === 0 ? (
                ...
                <motion.tr key={officer.id}
                  ...
                  <td className="p-4 font-mono text-blue-600 text-[12px] font-bold">{officer.badgeNo}</td>
                  <td className="p-4">
                    <div className="flex items-center gap-2.5">
                      <div className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center text-xs font-bold text-slate-600 border border-slate-200">
                  ...
                  <td className="p-4">
                    <span className={`px-2.5 py-1 rounded-xl text-[11px] font-bold border ${rankColors[officer.rank] || 'bg-slate-100 text-slate-600 border-slate-200'}`}>
                      {officer.rank}
                    </span>
                  </td>
                  ...
                  <td className="p-4 hidden lg:table-cell">
                    <span className="text-slate-600 bg-white px-2 py-0.5 rounded-lg text-xs border border-slate-200 shadow-sm">{officer.speciality}</span>
                  </td>
                  <td className="p-4">
        ...
        <div className="p-4 border-t border-slate-100 text-xs text-slate-500 font-medium bg-slate-50/50">
          Showing <span className="text-slate-800 font-bold">{filtered.length}</span> of <span className="text-slate-800 font-bold">{officers.length}</span> officers
        </div>
      </div>
