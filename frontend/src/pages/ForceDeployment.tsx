...
const statusStyles: Record<string, string> = {
  ...
  Completed: 'bg-slate-600/10 text-slate-500 border-slate-600/20',
};
...
export default function ForceDeployment() {
  ...
  const stats = [
    { label: 'Total Deployed', value: duties.length, color: 'text-slate-800', border: 'border-slate-200/40', bg: 'from-slate-50/60 to-white/60' },
    { label: 'On Duty', value: duties.filter(d => d.status === 'On Duty').length, color: 'text-orange-400', border: 'border-orange-500/20', bg: 'from-orange-900/20 to-white/60' },
    { label: 'Confirmed', value: duties.filter(d => d.status === 'Confirmed').length, color: 'text-emerald-400', border: 'border-emerald-500/20', bg: 'from-emerald-900/20 to-white/60' },
    { label: 'AI Assigned', value: duties.filter(d => d.aiGenerated).length, color: 'text-purple-400', border: 'border-purple-500/20', bg: 'from-purple-900/20 to-white/60' },
  ];
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="flex flex-wrap justify-between items-start gap-3">
        ...
        <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
          onClick={fetchDuties}
          className="px-4 py-2.5 bg-slate-50 hover:bg-slate-700 text-slate-600 rounded-2xl font-bold flex items-center gap-2 text-sm border border-slate-200 transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5 flex-1">
        {/* Zone Sidebar */}
        <div className="bg-white/60 backdrop-blur-xl border border-slate-200 rounded-3xl p-5 flex flex-col gap-4">
          <h3 className="text-base font-bold text-slate-800 flex items-center gap-2"><MapPin size={16} className="text-blue-400" /> Zone-wise Strength</h3>
          {loading ? (
                ...
                <div key={zone}>
                  <div className="flex justify-between text-xs font-medium mb-1.5">
                    <button onClick={() => setFilterZone(filterZone === zone ? 'All' : zone)}
                      className={`font-bold hover:text-slate-800 transition-colors ${filterZone === zone ? 'text-slate-800' : 'text-slate-500'}`}>
                      {zone.replace('Zone ', 'Z')}
                    </button>
          ...
          {!loading && duties.length > 0 && (
            <div className="mt-auto pt-4 border-t border-slate-200/50 space-y-1.5">
        ...
        {/* Main Table */}
        <div className="lg:col-span-3 bg-white/60 backdrop-blur-xl border border-slate-200 rounded-3xl overflow-hidden flex flex-col">
          {/* Toolbar */}
          <div className="p-4 border-b border-slate-200/50 flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[180px]">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="w-full pl-10 pr-4 py-2 bg-slate-50/50 border border-slate-200/40 rounded-2xl text-sm text-slate-800 placeholder-slate-600 focus:outline-none focus:border-blue-500 transition-all" />
            </div>
            <div className="flex gap-2 flex-wrap">
              {['All', 'Assigned', 'On Duty', 'Confirmed'].map(s => (
                <button key={s} onClick={() => setFilterStatus(s)}
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all ${filterStatus === s ? 'bg-blue-600 text-white border-blue-500' : 'bg-slate-50/50 text-slate-500 border-slate-200/50'}`}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/50 border-b border-slate-200/40 text-slate-500 text-[10px] uppercase tracking-widest font-bold">
                  ...
                  <motion.tr key={duty.id}
                    ...
                    <td className="px-5 py-3">
                      <select value={duty.status}
                        ...
                        className={`px-2.5 py-1 rounded-xl text-[11px] font-bold border appearance-none bg-transparent cursor-pointer focus:outline-none ${statusStyles[duty.status] || 'text-slate-600 border-slate-600'}`}>
          ...
          {/* Footer */}
          <div className="px-5 py-3 border-t border-slate-200/50 flex items-center justify-between">
            <p className="text-xs text-slate-500 font-medium">
              Showing <span className="text-slate-600 font-bold">{filtered.length}</span> of <span className="text-slate-600 font-bold">{duties.length}</span> deployments
            </p>
