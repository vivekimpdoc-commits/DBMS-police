...
export default function ForceDeployment() {
  ...
  const stats = [
    { label: 'Total Deployed', value: duties.length, color: 'text-white', border: 'border-white/10', bg: 'glass-card' },
    { label: 'On Duty', value: duties.filter(d => d.status === 'On Duty').length, color: 'text-orange-400', border: 'border-orange-500/20', bg: 'glass-card' },
    { label: 'Confirmed', value: duties.filter(d => d.status === 'Confirmed').length, color: 'text-emerald-400', border: 'border-emerald-500/20', bg: 'glass-card' },
    { label: 'AI Assigned', value: duties.filter(d => d.aiGenerated).length, color: 'text-purple-400', border: 'border-purple-500/20', bg: 'glass-card' },
  ];
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-5 flex-1">
        ...
        <div className="glass-card rounded-3xl p-5 flex flex-col gap-4">
          ...
          {loading ? (
            [...Array(4)].map((_, i) => <div key={i} className="h-10 bg-slate-800/40 rounded-xl animate-pulse" />)
          ...
          {!loading && duties.length > 0 && (
            <div className="mt-auto pt-4 border-t border-white/5 space-y-1.5">
              ...
              <div className="flex justify-between text-xs">
                <span className="text-slate-400">Events Covered</span>
                <span className="text-white font-bold">{new Set(duties.map(d => d.event.eventId)).size}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-slate-400">Zones Active</span>
        ...
        {/* Main Table */}
        <div className="lg:col-span-3 glass-card rounded-3xl overflow-hidden flex flex-col">
          {/* Toolbar */}
          <div className="p-4 border-b border-white/5 flex flex-wrap gap-3">
            <div className="relative flex-1 min-w-[180px]">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                ...
                className="w-full pl-10 pr-4 py-2 bg-slate-800/50 border border-slate-700 rounded-2xl text-sm text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-all" />
            </div>
            <div className="flex gap-2 flex-wrap">
              {['All', 'Assigned', 'On Duty', 'Confirmed'].map(s => (
                <button key={s} onClick={() => setFilterStatus(s)}
                  className={`px-3 py-1.5 rounded-xl text-xs font-bold border transition-all ${filterStatus === s ? 'bg-blue-600 text-white border-blue-500' : 'bg-slate-800/50 text-slate-400 border-slate-700'}`}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div className="overflow-x-auto flex-1">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-900/40 border-b border-white/5 text-slate-400 text-[10px] uppercase tracking-widest font-bold">
                  ...
                  <motion.tr key={duty.id}
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <p className="text-slate-400 text-[10px]">{duty.event.name}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3">
                    ...
                    <td className="px-5 py-3">
                      <select value={duty.status}
                        ...
                        className={`px-2.5 py-1 rounded-xl text-[11px] font-bold border appearance-none bg-slate-900 cursor-pointer focus:outline-none ${statusStyles[duty.status] || 'text-slate-400 border-slate-700'}`}>
          ...
          <div className="px-5 py-3 border-t border-white/5 flex items-center justify-between">
            <p className="text-xs text-slate-400 font-medium">
              Showing <span className="text-white font-bold">{filtered.length}</span> of <span className="text-white font-bold">{duties.length}</span> deployments
            </p>
